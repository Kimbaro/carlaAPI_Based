...
def game_loop(args):
    ...
    try:
        ...
        if args.agent == "Roaming":
            ...
            # Custom <-----
            print("타겟 아이디 : ", agent.vehicle.id)
            debug = world.world.debug
            list = agent._trace_route(agent.start_waypoint, agent.end_waypoint)
            for routeData in list:
                debug_start = routeData[0]  # return -> carla.Waypoint
                roadOption = routeData[1]  # return -> carla.RoadOption
                # print(debug_start)
                # print(roadOption, '\n')
                drawing_point.draw_point(debug, debug_start.transform.location, color=carla.Color(255, 0, 0),
                                         lt=60.0)
            # Custom <-----
