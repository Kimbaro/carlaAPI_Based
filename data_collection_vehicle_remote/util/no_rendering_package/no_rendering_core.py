...
import carla
from carla import TrafficLightState as tls  # <- 신호등 정보를 담고 있음.
...
import data_collection_vehicle_remote.util.no_rendering_package.module_config as config
import data_collection_vehicle_remote.util.no_rendering_package.color_palette as c

from data_collection_vehicle_remote.util.Sensors import GnssSensor
from data_collection_vehicle_remote.util.Sensors import IMUSensor
...
class TrafficLightSurfaces(object):
    ...
    def __init__(self):
        def make_surface(tl):
            ...
            surface.fill(c.COLOR_ALUMINIUM_5 if tl != 'h' else c.COLOR_ORANGE_2)
            if tl != 'h':
                hw = int(w / 2)
                off = c.COLOR_ALUMINIUM_4
                red = c.COLOR_SCARLET_RED_0
                yellow = c.COLOR_BUTTER_0
                green = c.COLOR_CHAMELEON_0
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points):
        ...
        if surface_pixel_per_meter > config.PIXELS_PER_METER:
            surface_pixel_per_meter = config.PIXELS_PER_METER
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        map_surface.fill(c.COLOR_ALUMINIUM_4)
        precision = 0.05
        ...
        def draw_arrow(surface, transform, color=c.COLOR_ALUMINIUM_2):
        ...
        def draw_traffic_signs(surface, font_surface, actor, color=c.COLOR_ALUMINIUM_2, trigger_color=c.COLOR_PLUM_0):
        ...
        def draw_topology(carla_topology, index):
            ...
            for waypoint in topology:
                ...
                PARKING_COLOR = c.COLOR_ALUMINIUM_4_5
                SHOULDER_COLOR = c.COLOR_ALUMINIUM_5
                SIDEWALK_COLOR = c.COLOR_ALUMINIUM_3
            ...
            for waypoints in set_waypoints:
                ...
                if len(polygon) > 2:
                    pygame.draw.polygon(map_surface, c.COLOR_ALUMINIUM_5, polygon, 5)
                    pygame.draw.polygon(map_surface, c.COLOR_ALUMINIUM_5, polygon)
        ...
        if self.show_spawn_points:
            for sp in carla_map.get_spawn_points():
                draw_arrow(map_surface, sp, color=c.COLOR_CHOCOLATE_0)
        ...
        stop_font_surface = font.render("STOP", False, c.COLOR_ALUMINIUM_2)
        stop_font_surface = pygame.transform.scale(
            stop_font_surface, (stop_font_surface.get_width(), stop_font_surface.get_height() * 2))

        yield_font_surface = font.render("YIELD", False, c.COLOR_ALUMINIUM_2)
        ...
        for ts_stop in stops:
            draw_traffic_signs(map_surface, stop_font_surface, ts_stop, trigger_color=c.COLOR_SCARLET_RED_1)

        for ts_yield in yields:
            draw_traffic_signs(map_surface, yield_font_surface, ts_yield, trigger_color=c.COLOR_ORANGE_1)
...
class World(object):
    ...
    def __init__(self, client, target, name, args, timeout):
        self.client = client
        self.name = name
        self.args = args
        ...
        # Hero actor
        self.hero_actor = target
        self.hud_target = target
    ...
    def select_hero_actor(self, map_mode_check=False):
        ...
        if map_mode_check is True:
            if len(hero_vehicles) > 0:
                self.hero_actor = None
                self.hero_transform = carla.Transform(carla.Location(x=0.0, y=0.0, z=0.0), carla.Rotation())
        elif map_mode_check is False:
            if len(hero_vehicles) > 0:
    ...
    def tick(self, clock, routeManager):
        ...
        if self.hero_actor is not None:
            self.hero_transform = self.hero_actor.get_transform()
            if self.hero_actor.get_location().x == 0.000000 and self.hero_actor.get_location().y == 0.000000 and self.hero_actor.get_location().z == 0.000000:
    ...
    def update_hud_info(self, clock):
        ...
        hero_mode_text = []
        sensor_text = []
        if self.hud_target is not None:
            hero_speed = self.hud_target.get_velocity()
            ...
            affected_speed_limit_text = self.hud_target.get_speed_limit()
            ...
            # ]
            w = self.town_map.get_waypoint(self.hud_target.get_location())
            hero_mode_text = [
                'Target ID:            %7d' % self.hud_target.id,
                'Target Speed:        %3d km/h' % hero_speed_text,
                'Status :',
                ' -Traffic Light: %11s' % affected_traffic_light_text,
                ' -Speed Limit:      %3d km/h' % affected_speed_limit_text,
                'OpenDRIVE :',
                ' -road_id : % 3d' % w.road_id,
                ' -lane_id : % 3d' % w.lane_id

            ]
            if self.gnss and self.imu is not None:
                compass = self.imu.compass
                ...
                sensor_text = [
                    'IMU :',
                    ' -compass: % 10.0f\N{DEGREE SIGN} % 2s' % (compass, heading),
                    ' -accelero: %1.1f, %1.1f, %1.1f' % (self.imu.accelerometer),
                    ' -gyroscop: %1.1f, %1.1f, %1.1f' % (self.imu.gyroscope),
                    'GNSS :',
                    ' - lat: % 2.6f' % self.gnss.lat,
                    ' - lon: % 3.6f' % self.gnss.lon
                ]
        else:
    ...
    def _render_traffic_lights(self, surface, list_tl, world_to_pixel):
        ...
        for tl in list_tl:
            ...
            if self.args.show_triggers:
                ...
                corners = [world_to_pixel(p) for p in corners]
                pygame.draw.lines(surface, c.COLOR_BUTTER_1, True, corners, 2)
    ...
    def _render_speed_limits(self, surface, list_sl, world_to_pixel, world_to_pixel_width):
        ...
        for sl in list_sl:
            ...
            pygame.draw.circle(surface, c.COLOR_SCARLET_RED_1, (x, y), radius)
            pygame.draw.circle(surface, c.COLOR_ALUMINIUM_0, (x, y), white_circle_radius)

            limit = sl.type_id.split('.')[2]
            font_surface = font.render(limit, True, c.COLOR_ALUMINIUM_5)

            if self.args.show_triggers:
                corners = Util.get_bounding_box(sl)
                corners = [world_to_pixel(p) for p in corners]
                pygame.draw.lines(surface, c.COLOR_PLUM_2, True, corners, 2)
    ...
    def _render_destination(self, surface, routeManager, world_to_pixel):
        """Renders the walkers' bounding boxes"""
        if routeManager.routeDestinationList is not None:
            for w in routeManager.routeDestinationList:
                color = c.COLOR_RED
                # Compute bounding box points
                corners = [
                    carla.Location(x=w.transform.location.x - 1, y=w.transform.location.y - 1),
                    carla.Location(x=w.transform.location.x, y=w.transform.location.y - 1),
                    carla.Location(x=w.transform.location.x, y=w.transform.location.y),
                    carla.Location(x=w.transform.location.x - 1, y=w.transform.location.y)]

                # w.transform(corners)
                corners = [world_to_pixel(p) for p in corners]
                pygame.draw.polygon(surface, color, corners)

        if routeManager.move_key is not None:
            w = routeManager.move_key
            color = c.COLOR_CUSTOM_0
            # Compute bounding box points
            corners = [
                carla.Location(x=w.transform.location.x - 1, y=w.transform.location.y - 1),
                carla.Location(x=w.transform.location.x, y=w.transform.location.y - 1),
    ...
    def _render_route(self, surface, routePlannerList, world_to_pixel):
        color = c.COLOR_CUSTOM_3
        for routeData in routePlannerList:
            debug_start = routeData[0]  # return -> carla.Waypoint
            lo = debug_start.transform.location
            # Compute bounding box points
            corners = [
                carla.Location(x=lo.x - 1, y=lo.y - 1),
                carla.Location(x=lo.x, y=lo.y - 1),
                carla.Location(x=lo.x, y=lo.y),
                carla.Location(x=lo.x - 1, y=lo.y)]
    ...
    def render_actors(self, surface, vehicles, traffic_lights, speed_limits, walkers, routeManager):
        ...
        self._render_destination(surface, routeManager, self.map_image.world_to_pixel)
        self._render_route(surface, routeManager.routePlannerList, self.map_image.world_to_pixel)
    ...
    def render(self, display, routeManager):
        ...
        self.routeManager = routeManager
        # Render Actors
        self.actors_surface.fill(c.COLOR_BLACK)
        self.render_actors(
            self.actors_surface,
            vehicles,
            traffic_lights,
        ...
        if self.hero_actor is not None:
            ...
            translation_offset = (
                hero_location_screen[
                    0] - self.hero_surface.get_width() / 2 + hero_front.x * config.PIXELS_AHEAD_VEHICLE,
                (hero_location_screen[
                     1] - self.hero_surface.get_height() / 2 + hero_front.y * config.PIXELS_AHEAD_VEHICLE))
            ...
            self.hero_surface.fill(c.COLOR_ALUMINIUM_4)
