...
import numpy as np
import carla
from data_collection_vehicle_remote.agents.navigation.agent import Agent
from data_collection_vehicle_remote.agents.navigation.local_planner_behavior import LocalPlanner, RoadOption
from data_collection_vehicle_remote.agents.navigation.global_route_planner import GlobalRoutePlanner
from data_collection_vehicle_remote.agents.navigation.global_route_planner_dao import GlobalRoutePlannerDAO
from data_collection_vehicle_remote.agents.navigation.types_behavior import Cautious, Aggressive, Normal
...
class BehaviorAgent(Agent):
    ...
    def emergency_speed_control(self, remote=False, speed=0):
        """
        대상차량의 속도를 임의로 제어함. update_information() 메서드에서 반영됨.
        :param remote: True인 경우 설정정보 적용
        :param speed: 적용시킬 속도
        :return:
        """
        if remote:
    ...
    def update_information(self):
        ...
        if self.remote is True:
            # print("emergency_speed_control !!!! : ", self.speed)
            self.speed_limit = self.speed
        else:
        ...
        if self.ignore_traffic_light:
            ...
            # print("check light_state : ", self.light_state)

        # if self.log_check is True and self.speed != 0.0:  # 터미널에 차량 속도, 신호등, 위치를 기록함.
        #     draw_waypoints(self.vehicle.get_world(), [self.incoming_waypoint])  # 초록등일때, 주행중 타겟차량의 다음 경로 웨이포인트 드로잉
        #     print("speed :", self.speed, "traffic_light :", self.light_state, "location :", self.vehicle.get_location())
